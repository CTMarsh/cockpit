...
deployHistoryRoutes.openapi(eventsRoute, async (c) => {
  ...
  let events;
  if (deployment && namespace) events = await sql`SELECT * FROM deployment_events WHERE deployment = ${deployment} AND namespace = ${namespace} ORDER BY created_at DESC LIMIT ${limit}`;
  else if (namespace) events = await sql`SELECT * FROM deployment_events WHERE namespace = ${namespace} ORDER BY created_at DESC LIMIT ${limit}`;
  else events = await sql`SELECT * FROM deployment_events ORDER BY created_at DESC LIMIT ${limit} OFFSET ${offset}`;
...
deployHistoryRoutes.openapi(recordRoute, async (c) => {
  ...
  await sql`INSERT INTO deployment_events (deployment, namespace, action, old_value, new_value, triggered_by) VALUES (${body.deployment}, ${body.namespace}, ${body.action}, ${body.old_value || ""}, ${body.new_value || ""}, ${body.triggered_by || "cockpit"})`;
  return c.json({ ok: true }, 201);
});

export async function recordDeployEvent(deployment: string, namespace: string, action: string, oldValue = "", newValue = "", triggeredBy = "cockpit") {
  await sql`INSERT INTO deployment_events (deployment, namespace, action, old_value, new_value, triggered_by) VALUES (${deployment}, ${namespace}, ${action}, ${oldValue}, ${newValue}, ${triggeredBy})`;
}
